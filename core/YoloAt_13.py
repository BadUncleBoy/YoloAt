...
class YoloAt_13(object):
    ...
    def _mask_branch(self,route_1,route_2):
        route_1 = common._fixed_padding(route_1,3)
        route_1 = slim.conv2d(route_1, 512, 3, stride=2, padding='VALID', normalizer_fn=slim.batch_norm,
                                normalizer_params=self.batch_norm_params,
                                biases_initializer=None,
                                activation_fn=lambda x: tf.nn.leaky_relu(x, alpha=self._LEAKY_RELU))
        route = tf.concat([route_1,route_2], axis=-1)
        route = common._fixed_padding(route,3)#26*26*1024
        mask  = slim.conv2d(route, 1024, 3, stride=2, padding='VALID', normalizer_fn=slim.batch_norm,
                                normalizer_params=self.batch_norm_params,
                                biases_initializer=None,
                                activation_fn=lambda x: tf.nn.sigmoid(x))
        return mask #13*13*1024
    ...
    def forward(self, inputs, is_training=False, reuse=False):
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm, common._fixed_padding],reuse=reuse):
            with slim.arg_scope([slim.conv2d], normalizer_fn=slim.batch_norm,
                ...
                with tf.variable_scope('yoloAt-13'):
                    mask        = self._mask_branch(route_1, route_2)
